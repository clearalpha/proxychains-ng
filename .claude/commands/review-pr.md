...
Perform a code review of a pull request using specialist subagents.

## Target PR

Determine which pull request to review:

- If the user gives a PR number or URL, use it.
- Otherwise resolve the PR for the current branch with `gh pr view --json number,url,headRepositoryOwner,headRepository,title,body,labels` (or plain `gh pr view`). The `title`, `body`, and `labels` fields add no extra call here and are reused by the walkthrough pass, saving a round-trip.

Set `<REPO>` (the `owner/repo` slug) and `<PR_NUMBER>` from that, and substitute them into the `gh` commands below. The repository should already be checked out locally.

## Available tools

Use `Bash` (for `gh` CLI commands), `Read`, `Glob`, `Grep`, and `Task` (to dispatch the specialist reviewer subagents). To post inline review comments, use the `mcp__github_inline_comment__create_inline_comment` MCP tool **when it is available**; otherwise post inline and summary comments with `gh api` / `gh pr comment`. You do not need `WebFetch` or `WebSearch` for this workflow.
...
## Fast-path: use review metadata when provided

A caller (for example, a CI pre-check job) may provide metadata in the prompt arguments under the heading "Pre-check metadata (from CI — do NOT re-fetch)". This is optional — when it is absent, gather everything via `gh` as described in the sections below. When this metadata **is** present:
...
6. **If `REVIEW_CONTEXT_DIR` is present and non-empty** — the pre-check has already fetched the git history and prior-PR feedback into files under that directory. Pass `REVIEW_CONTEXT_DIR` verbatim to `git-history-reviewer` and `prior-pr-reviewer` when you dispatch them; each reads its own files and issues no `gh api` calls. Two companion fields tell you whether dispatching is worthwhile at all: `HISTORY_COMMIT_COUNT` (unique prior commits touching these files) and `PRIOR_PR_NUMBERS` (space-separated prior PRs whose feedback was collected). A count of `0` or an empty list means that lens has nothing to work with — skip it.
7. **You still need to fetch existing inline comments** (step 1 of "Pre-review: gather context") to avoid posting duplicate findings. This is the one API call that cannot be skipped.
...
- **Verification pass**: 1 turn (one parallel finding-verifier batch; only when eligible findings exist)
...
The **walkthrough** must add almost nothing to this budget: for medium/large PRs it is a parallel subagent (no extra orchestrator turns beyond dispatch); for **small** PRs you generate it inline, so keep it cheap — reuse the `title`/`body`/`labels` from PR resolution, reuse the issue-comments fetch from the fingerprint check, resolve the feature flags **without a shell probe** (they default disabled — see "Feature flags"), and reuse the files you already read for the review. Post the walkthrough **immediately after generating it** (don't defer it behind the findings work) so a tight turn limit (e.g. a CI `--max-turns` cap) can't strand it unposted.
...
| **Medium** | ≤800 lines AND ≤15 files (but not Small) | **4 subagents** — code-quality-reviewer + security-code-reviewer + git-history-reviewer + prior-pr-reviewer |
| **Large** | >800 lines OR >15 files | **All 7 subagents** |
...
**Medium PR review**: Dispatch code-quality-reviewer, security-code-reviewer, git-history-reviewer, and prior-pr-reviewer. Handle performance, test-coverage, and documentation concerns yourself during post-processing, but only flag obvious issues. The two history lenses run at Medium as well as Large because wrong-calculation-basis and repeated-mistake defects — the class file-only review cannot see — occur in medium-sized PRs just as often.

## Walkthrough (comprehension pass)

Before posting findings, post a short narrative **walkthrough** that orients the human reviewer. This is a different job from the review: it *explains* the change, it does **not** find issues (that stays the reviewers' job). It is always exactly **one** PR comment, marked `<!-- claude-walkthrough -->` and updated in place — never a second findings stream. Run it on **every** PR you review, and post it **before** the findings review so the human is oriented first.

> The walkthrough never fetches the full diff. It reuses the changed-file list, per-file metadata, and `PR_SIZE` already gathered (or supplied via pre-check metadata), plus the PR title/body and light `Read`s.

### Feature flags (Phase 2 & 3, off by default)

Two optional blocks are gated behind flags that **default to disabled**. Resolve them **without spending a turn on a shell probe**:

- If the pre-check metadata block contains `PR_WALKTHROUGH_PROVENANCE` / `PR_WALKTHROUGH_DIAGRAM` set to `1`, `true`, or `on` (case-insensitive), **enable** the corresponding block — `PROVENANCE` → the Provenance block (Phase 2); `DIAGRAM` → the Flow diagram block (Phase 3).
- Otherwise treat **both as disabled** (the default).

**Do not run `printenv`/`echo` to read these.** The CI review action permits only `gh api`, `gh pr diff`, `gh pr view`, and `gh pr comment` as `Bash`, so an env-var probe is denied and wastes turns retrying. Only in an unrestricted local session may you read them directly (`printenv PR_WALKTHROUGH_PROVENANCE PR_WALKTHROUGH_DIAGRAM`); if that is not trivially available, default to disabled.

### When to run

Obey the same fingerprint decision already computed in "Pre-review: check if review is needed":

| Review decision | Walkthrough |
|---|---|
| **Skip** (no diff change / merge-only) | **Skip the walkthrough too** — post or change nothing. |
| **Full review** | Regenerate the walkthrough for the current PR state. |
| **Incremental review** | Regenerate the walkthrough for the current PR state — it always describes the whole PR, not just the newly changed files. |

### Gather walkthrough context

Reuse context you already have — do **not** add round-trips:

- PR **title / body / labels**: already fetched during PR resolution ("Target PR"). Fetch again **only** if the PR was supplied by number and you never ran `gh pr view` — then `gh pr view <PR_NUMBER> --json title,body,labels` once (this is **not** a diff fetch).
- Changed-file list, per-file metadata, `PR_SIZE`: already in hand from pre-review (or pre-check metadata) — do not re-fetch.
- For a **small** PR you will read the changed files for the inline review anyway — reuse those reads for the walkthrough; do not re-read.

### Generate the walkthrough body

The output contract — exact structure, the deterministic effort/risk score, relatedness bucketing, and the gated Provenance / Flow blocks — is defined once in `agents/pr-walkthrough-writer.md` ("Output contract"). Generate a body that follows it exactly. The body must contain no findings, severities, or inline-comment syntax, and its first line must be the literal marker `<!-- claude-walkthrough -->` with a real `!` — never an escaped `<\!--` (which renders visibly and breaks the in-place-update search).

- **Small PRs:** generate the walkthrough **inline** as the orchestrator (mirrors the inline small-PR review). `Read` the few changed files as needed and apply the contract directly, honoring the two resolved flag states.
- **Medium / Large PRs:** dispatch **`pr-walkthrough-writer`** as **one additional parallel `Task`**, in the same batch as the reviewer subagents (it runs *alongside* them, replacing none). Pass it: the PR title/body, the changed-file list + per-file metadata, `PR_SIZE`, the changed path list, the PR labels, and the two resolved flag states as `PROVENANCE: enabled|disabled` and `DIAGRAM: enabled|disabled`. It returns the walkthrough body.

One pass only — the walkthrough is a single generation, not a multi-agent analysis.

### Post the walkthrough (in place, single comment)

Post (or update) the walkthrough **before** posting the findings review.

1. Find any existing **bot** walkthrough comment. **Reuse the issue comments you already fetched for the fingerprint check** ("Pre-review: check if review is needed") — scan them for the marker in that same set instead of making a second `issues/.../comments` call. Match on the stable slug `claude-walkthrough`, **not** the full `<!-- … -->` form: a generated marker can arrive with the `!` escaped as `<\!--`, which a full-form match would miss — leaving a duplicate walkthrough on every re-run. The slug has no escapable characters, so it matches either form. Only if you have not already fetched the issue comments, fetch them now:

   `gh api repos/<REPO>/issues/<PR_NUMBER>/comments --paginate --jq '.[] | select(.user.type == "Bot") | select(.body | contains("claude-walkthrough")) | {id}'`

   Always filter by `.user.type == "Bot"` so a human cannot spoof the marker.
2. **Do not use the `Write` tool or `mktemp`** — neither is permitted in the CI review action (allowed `Bash` is only `gh api` / `gh pr diff` / `gh pr view` / `gh pr comment`). Pass the body **inline via a quoted heredoc** (`<<'WALKTHROUGH_EOF'`), which keeps every special character — backticks, `!`, `$`, quotes — literal, and so also keeps the marker an invisible HTML comment. **Normalize the marker first:** the first line must be exactly `<!-- claude-walkthrough -->`; if generation produced `<\!-- claude-walkthrough -->` (escaped `!`), fix it to `<!--`.
3. **If a marker comment is found**, update it in place so there is always exactly one current walkthrough:

   ```
   gh api --method PATCH repos/<REPO>/issues/comments/<COMMENT_ID> -F body=@- <<'WALKTHROUGH_EOF'
   <walkthrough body>
   WALKTHROUGH_EOF
   ```

4. **If not found**, post a new top-level comment:

   ```
   gh pr comment <PR_NUMBER> --body-file - <<'WALKTHROUGH_EOF'
   <walkthrough body>
   WALKTHROUGH_EOF
   ```
5. The walkthrough is a **separate** comment from the findings review (`<!-- claude-review-fingerprint ... -->`); they never merge.
...
In the **same parallel `Task` batch**, also dispatch `pr-walkthrough-writer` (the comprehension pass — see "Walkthrough (comprehension pass)"). It runs alongside the reviewers, not instead of one, and returns the walkthrough body rather than findings.
...
- **git-history-reviewer** — production source files only; pass `<REPO>`, the PR number, the head SHA, and `REVIEW_CONTEXT_DIR` when present. Skip this reviewer when `HISTORY_COMMIT_COUNT` is `0`, or when every file in its scope has status `added` (brand-new files have no history to contradict).
- **prior-pr-reviewer** — all changed files; pass `<REPO>`, the PR number, the head SHA, and `REVIEW_CONTEXT_DIR` + `PRIOR_PR_NUMBERS` when present. Skip this reviewer when `PRIOR_PR_NUMBERS` is present and empty (no prior PR touched these files, so there is no feedback to carry forward).
...
- `REVIEW_CONTEXT_DIR`, verbatim, whenever the pre-check metadata provides it
- Instruction to use `Read` to examine individual source files

## Post-review: filter and post

> Post the **walkthrough** comment first (see "Walkthrough (comprehension pass)"), then the findings below. The two are always separate comments.
...
8. **Verification pass** (see below): adversarially verify citation-based and high-severity findings before posting

### Verification pass (dispatch finding-verifier)

After filters 1–7, dispatch one **`finding-verifier`** subagent per finding, in a single parallel `Task` batch, for every finding that matches ANY of:

- produced by **git-history-reviewer** or **prior-pr-reviewer** (their citations are the most valuable findings when right and the most hallucination-prone when wrong — verification is **mandatory**, never skip it for these)
- severity **HIGH or CRITICAL**, from any reviewer
- produced during an **incremental review**, at any severity (see the staleness note below)

Pass each verifier: the finding verbatim, which reviewer produced it, `<REPO>`, the PR number, and the current head SHA. Each verifier returns `{"score": 0-100, "verdict": "..."}`. **Drop every verified finding scoring below 70.** MEDIUM/LOW findings from the file-scope reviewers (quality, security, performance, test-coverage, documentation) skip verification for cost, but remain subject to the observations cap below.

**Incremental-review staleness**: a finding made against a mid-branch commit can be reconciled minutes later by the branch merging its base or a companion PR (same ticket, sibling module) — posting it then is noise, and cross-file contract-mismatch findings (stack↔module, caller↔interface) are the classic case. The verifier's currency check (re-check at the CURRENT head, not the trigger commit) exists precisely for this; that is why incremental findings always verify. If the verifier finds the PR head has moved past the state you reviewed and the finding no longer holds, it scores ≤ 25 and the finding is dropped silently.

### Severity calibration

Before grouping by severity, recheck one rule: a finding that demonstrates a **reproducible malfunction** — a crash, hang/infinite loop, data corruption, or wrong computed result on a reachable input — is never LOW or INFO. Classify it at least MEDIUM, and HIGH (inline) if user-facing or on a money/risk path. Severity tracks impact, not how small the fix is; a one-line guard against a tab-hanging loop is still a HIGH-impact finding.

After filtering and verification, separate surviving findings into two groups by severity:
...
- **Summary-only group** (MEDIUM or LOW; INFO findings are dropped, not posted): Do NOT create inline comments for these. Instead, collect them into a single summary section appended to the review comment (see format below).
...
If there are any MEDIUM/LOW findings after filtering, append a collapsible section to your review comment. **Hard cap: 5 rows.** Drop INFO-severity findings entirely — do not post them anywhere. If more than 5 MEDIUM/LOW findings survive, keep the 5 most consequential (closest to correctness impact) and discard the rest; a 10-row observations table buries the one row that matters and trains readers to skip the section.
