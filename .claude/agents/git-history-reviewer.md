...
- A commit's diff, scoped to ONE file: `gh api repos/<REPO>/commits/<sha> --jq '.files[] | select(.filename=="<file>") | .patch'` — always path-filter. The unfiltered response carries every file in the commit and reaches ~1 MB in a monorepo whose history is large squash-merges; the filter costs nothing extra over the wire and cuts what you read by an order of magnitude or more.
...
Bound the work by VOLUME, not commit count. First build the union of candidate commits across all modified files — files changed together usually share commits, so deduping here can cut the fetch count several-fold. List them with the metadata-only call above (a few hundred bytes), then fetch patches only for commits whose message suggests relevance, always path-filtered, stopping once you have read roughly 200 KB of patch text. Prioritise files with the largest deletions and the most safety-critical logic (money, risk, auth, data migration). A repo whose history is a few very large squash-merges will exhaust the budget in two or three commits; that is correct behaviour, not a failure — report on what you read and say what you did not reach.
